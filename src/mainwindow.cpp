	...
	ui_(new Ui::MainWindow)
{
	...
	connect(ui_->fractalWidget, &FractalWidget::sizeChanged, ui_->lineSize, &SizeEdit::setValue);
	connect(ui_->lineSize, &SizeEdit::sizeChanged, this, QOverload<const QSize &>::of(&MainWindow::resize));
	...
	connect(ui_->spinDamping, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &MainWindow::on_settingsChanged);
	...
	ui_->lineSize->setValue(defaults.size);
	ui_->spinIterations->setValue(defaults.maxIterations);
	ui_->spinDamping->setValue(defaults.damping);
...
void MainWindow::on_settingsChanged()
{
	...
	params.damping = ui_->spinDamping->value();
	...
	for (quint8 i = 0; i < NRT; ++i) {
		...
		if (i < degree) {
			params.roots.append(rootEdits[i]->value());
		}
	}
...
void MainWindow::on_rootMoved(quint8 index, complex value)
{
	// Update settings
	if (index < NRT) {
		rootEdits[index]->setValue(value);
	}
}
